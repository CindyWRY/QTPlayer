...
static int upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx) {
    ...
    /* This should only happen if we are not using avfilter... */
     ...
     //转换为目标格式AV_PIX_FMT_BGRA，对应SDL_PIXELFORMAT_BGRA32
        case SDL_PIXELFORMAT_UNKNOWN:
            *img_convert_ctx = sws_getCachedContext(*img_convert_ctx,
            ...
            break;
        // frame格式对应SDL_PIXELFORMAT_IYUV，不用进行图像格式转换，
        //调用SDL_UpdateYUVTexture()更新SDL texture
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            /* compute nominal last_duration */// 上一帧播放时长：vp->pts - lastvp->pts
            ...
            if (frame_queue_nb_remaining(&is->pictq) > 1) {
                // 队列中未显示帧数>1(只有一帧则不考虑丢帧)
                // 下一帧：下一待显示的帧
                Frame *nextvp = frame_queue_peek_next(&is->pictq);

                // 当前帧vp播放时长 = nextvp->pts - vp->pts
                ...
                if(!is->step && (framedrop>0 ||
                   ...
                   // framedrop丢帧处理有两处：1) packet入队列前，2) frame未及时显示(此处)
                    is->frame_drops_late++;
                    // 删除上一帧已显示帧，即删除lastvp，读指针加1(从lastvp更新到vp)
                    frame_queue_next(&is->pictq);
                    goto retry;
                }
            }
            ...
            // 删除当前读指针元素，读指针+1。若未丢帧，
            //读指针从lastvp更新到vp；若有丢帧，
            //   读指针从vp更新到nextvp
            frame_queue_next(&is->pictq);
            is->force_refresh = 1;
 ...
 * value.
 音频重采样在audio_decode_frame()中实现，
 audio_decode_frame()就是从音频frame队列中取出一个frame，
 按指定格式经过重采样后输出。
 */
...
/*audio_open()函数填入期望的音频参数，打开音频设备后，
将实际的音频参数存入输出参数is->audio_tgt中，
后面音频播放线程用会用到此参数，
使用此参数将原始音频数据重采样，
转换为音频设备支持的格式。*/
