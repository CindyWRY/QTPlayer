...
void MainWindow::initUI()
{
	...
	//对QObject组件调用installEvenFilter函数，
	//即为组件安装过滤器，才能使用事件过滤器这个机制。
...
//锟斤拷锟矫伙拷锟较讹拷锟斤拷锟介，锟斤拷锟斤拷sliderMoved 锟脚号★拷
    ...
    connect(m_decoder, SIGNAL(playStateChanged(Decoder::PlayState)),  this, SLOT(playStateChanged(Decoder::PlayState)));
...
void MainWindow::initTray()
{
    ...
    QAction *minimizeAction = new QAction(tr("鏈€灏忓寲 (&I)"), this);
    connect(minimizeAction, SIGNAL(triggered()), this, SLOT(hide()));
    QAction *restoreAction = new QAction(tr("杩樺師 (&R)"), this);
    connect(restoreAction, SIGNAL(triggered()), this, SLOT(showNormal()));
    QAction *quitAction = new QAction(tr("閫€鍑?(&Q)"), this);
...
bool MainWindow::eventFilter(QObject *object, QEvent *event)
{
	...
	if (object == ui->videoProgressSlider) {
        if (event->type() == QEvent::MouseButtonPress) {//鼠标进度条按下
            QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);//强制转化为鼠标事件
...
void MainWindow::playVideo(QString file)
{
    ...
    if (m_currentPlayType == "video") {
        ...
        ui->titleLable->setText(QString("褰撳墠鎾斁锛?1").arg(getFilenameFromPath(file)));
    }
...
{
    QString filename = QFileDialog::getSaveFileName(this, "淇濆瓨鎴浘", "/", "(*.jpg)");
...
void MainWindow::timerSlot()
{
    if (QObject::sender() == m_menuTimer) {
        if (m_menuIsVisible && m_playState == Decoder::PLAYING) {
            if (isFullScreen()) {
